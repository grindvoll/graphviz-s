...
trait DotConstructors extends Dot {
  ...
  def handle_attributeList(attrs1: Option[Seq[T_AttributeAssignment]], attrs2: Option[Seq[T_AttributeAssignment]]): T_AttributeList
...
trait DotParser extends DotConstructors with RegexParsers {
  ...
  lazy val statement: Parser[T_Statement] = assignment_statement |
    attribute_statement |
    subgraph |
    edge_statement |
    node_statement
  ...
  lazy val attribute_list: Parser[T_AttributeList] =
    l_box ~ attributes ~ r_box ~ attributes ^^ { case (_ ~ al1 ~ _ ~ al2) => handle_attributeList(al1, al2) }

  lazy val attributes: Parser[Option[Seq[T_AttributeAssignment]]] = repsep(attribute_assignment, comma) ^^ { x => noneOnEmpty(x) }
  ...
  lazy val edge_statement: Parser[T_EdgeStatement] = node ~ edge_rhs ~ attribute_list.? ^^ { case n ~ ns ~ as => handle_edgeStatement(n, ns, as) }
  ...
  lazy val port: Parser[T_Port] = (colon ~> id ~ (colon ~> compass_pt).? ^^ { case id ~ cp => handle_port(Some(id), cp) }) |
